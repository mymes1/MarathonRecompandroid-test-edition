...
namespace
{
    ...
    const char* const kIcon[TC_COUNT] =
    {
        "A", // stick (unused)
        "A", "B", "X", "Y",
        "LB", "RB",
    ...
    // Marathon has no shared button-glyph atlas (button_guide is Unleashed-only), so
    // the on-screen controls draw plain text labels on their backing discs instead.
    void DrawGlyph(ImDrawList* dl, ImVec2 c, float halfW, float halfH, const char* label, int alpha)
    {
        ImFont* font = ImGui::GetFont();
        if (font == nullptr)
            return;

        // Glyph size tracks the smaller backing dimension so labels never overflow
    ...
    void DrawFaceButton(ImDrawList* dl, const std::vector<ImVec2>& pts, ImVec2 c, float r,
        const char* label, uint16_t bit, XAMINPUT_GAMEPAD& st)
    {
    ...
    bool DrawWideButton(ImDrawList* dl, const std::vector<ImVec2>& pts, ImVec2 c,
        float halfW, float halfH, float glyphHalfW, float glyphHalfH, const char* label)
    {
...
{
    g_cutsceneActive.store(active, std::memory_order_relaxed);
}
...
void TouchControls::Draw()
{
    ...
    if (!g_edit)
    {
        ...
        // Cutscenes are in-engine cutscene scenes (edge-triggered ctor/dtor hooks)
        // plus live WMV playback such as the attract movie (per-frame stamp from
        ...
        DrawFaceButton(dl, pts, ElemRectOf(TC_A, vw, vh).c, faceR, "A", XAMINPUT_GAMEPAD_A, st);
        DrawFaceButton(dl, pts, ElemRectOf(TC_B, vw, vh).c, faceR, "B", XAMINPUT_GAMEPAD_B, st);
        DrawFaceButton(dl, pts, ElemRectOf(TC_X, vw, vh).c, faceR, "X", XAMINPUT_GAMEPAD_X, st);
        DrawFaceButton(dl, pts, ElemRectOf(TC_Y, vw, vh).c, faceR, "Y", XAMINPUT_GAMEPAD_Y, st);

        // ---- Shoulders ----
        const float shHW = SHOULDER_HW * vh * g_layout.scale;
        const float shHH = SHOULDER_HH * vh * g_layout.scale;
        if (DrawWideButton(dl, pts, ElemRectOf(TC_LB, vw, vh).c, shHW, shHH, shHW * 0.75f, shHH * 0.85f, "LB"))
            st.wButtons |= XAMINPUT_GAMEPAD_LEFT_SHOULDER;
        if (DrawWideButton(dl, pts, ElemRectOf(TC_RB, vw, vh).c, shHW, shHH, shHW * 0.75f, shHH * 0.85f, "RB"))
            st.wButtons |= XAMINPUT_GAMEPAD_RIGHT_SHOULDER;

        // ---- Triggers ----
        if (DrawWideButton(dl, pts, ElemRectOf(TC_LT, vw, vh).c, shHW, shHH, shHH * 0.95f, shHH * 0.95f, "LT"))
            st.bLeftTrigger = 255;
        if (DrawWideButton(dl, pts, ElemRectOf(TC_RT, vw, vh).c, shHW, shHH, shHH * 0.95f, shHH * 0.95f, "RT"))
            st.bRightTrigger = 255;

        // ---- Start / Back ----
        const float menuHW = MENU_HW * vh * g_layout.scale;
        const float menuHH = MENU_HH * vh * g_layout.scale;
        if (DrawWideButton(dl, pts, ElemRectOf(TC_START, vw, vh).c, menuHW, menuHH, menuHW, menuHH, "START"))
            st.wButtons |= XAMINPUT_GAMEPAD_START;

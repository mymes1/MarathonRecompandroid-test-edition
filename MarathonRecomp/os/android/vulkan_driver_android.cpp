...
// The APK may ship source-built Mesa Turnip driver builds for the Adreno GPUs this port
// targets (the reference port bundles Mesa 26.1.4 Turnip from the same cc9b2874cb69
// snapshot). The FILENAMEs stay stable so existing installs keep working: provisioning
// compares the complete asset contents, so clean and WFM binaries (which happen to have
// the same size) still replace one another. The Android Render Mode option selects the
// normal/GMEM ("none") or Sysmem ("sysmem") TU_DEBUG default. An external
// driver_import/tu_debug.txt remains a diagnostic override. "flushall" enables Mesa's
// real full per-draw flush and causes a massive FPS hit.
//
// No driver binaries are committed to this repository: drop community Turnip builds into
...
static void ProcessDriverImportDir(const std::filesystem::path &turnipDir)
{
    ...
    WriteTextFile(importDir / "readme.txt",
        ...
        "instead. On Mali you can experiment with importing a PanVK build (a\n"
        "plain .so) here. On Samsung Xclipse, drop an ExynosTools package zip\n"
        "from https://github.com/WearyConcern1165/ExynosTools into this folder\n"
        "to use its compatibility layer instead of the plain system driver.\n");
...
static std::string GetImportedDriverName(const std::string &turnipDir)
{
    ...
    if (legacyName != BUNDLED_DRIVER_NAME && legacyName != VAUZI_710_DRIVER_NAME &&
        legacyName != EXPERIMENTAL_A725_DRIVER_NAME)
    {

    ...
    const std::filesystem::path & GetDataRoot()
    {
        static std::filesystem::path root = []() -> std::filesystem::path
        {
            ...
            std::filesystem::path legacy = !GetInternalFilesDir().empty()
                ? GetInternalFilesDir() / "MarathonRecomp"
            ...
            if (!media.empty())
            {
                std::filesystem::path mediaRoot = media / "MarathonRecomp";
                bool externalPopulated = !external.empty() &&
                    std::filesystem::exists(external / "MarathonRecomp" / "game", ec);
            ...
            if (!external.empty())
            {
                std::filesystem::path result = external / "MarathonRecomp";
                ...
                if (!media.empty())
                {
                    std::filesystem::create_directories(media / "MarathonRecomp", ec);
            ...
            std::filesystem::path result = GetInternalFilesDir() / "MarathonRecomp";
            std::filesystem::create_directories(result, ec);
            return result;
        }();
